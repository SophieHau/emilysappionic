...
class ChatBoard extends React.Component {
        ...
        snaphot.docChanges().forEach((change) => {
          if (change.type === 'added') {
            const newMessage = {
              ...
              createdAt: new Date(
                change.doc.data().createdAt * 1000
              ).toString(),
              author: change.doc.data().author,
            };
            firestore
              .collection('users')
              .doc(change.doc.data().author.id)
              .get()
              .then((author) => {
  ...
  saveToken = async () => {
    try {
      ...
      if (err.code === 'messaging/token-unsubscribe-failed') this.saveToken();
    }
  };
              ...
              return (
                <article key={message.id} className="dib w-100">
                  <div key={message.id} className="dtc v-mid pl5 tr fr">
                    <h1
                      key={message.id}
                      className="mr2 f6 tl fw4 br4 ph3 pv2 dib bg-washed-green mid-gray shadow-4 fr"
                    >
                      {message.content}{' '}
                      <p className="tr f7 fw2 mb0 mt1 black-60">
                        {message.createdAt.substring(4, 10)} |{' '}
                        {message.createdAt.substring(16, 21)}
                      </p>
                    </h1>
                  </div>
                </article>
              ...
              return (
                <article key={message.id} className="w-100 dib">
                  <div key={message.id} className="dtc v-mid pr5">
                    <h1
                      key={message.id}
                      className="f6 tl fw4 br4 ph3 pv2 mh2 dib mid-gray shadow-4"
                    >
                      {message.content}{' '}
                      <p
                        className={`tr f7 fw4 ${message.color} mb0 mt2 black-60`}
                      >
                        {message.author}{' '}
                        <span className="f7 fw2 black-60">
                          {' '}
                          {message.createdAt.substring(4, 10)} |{' '}
                          {message.createdAt.substring(16, 21)}
                        </span>
